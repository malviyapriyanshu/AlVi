...
export const fibonacciDP = (n: number): AnimationStep[] => {
  ...
  steps.push({ type: 'table_init', table: [0, 1], explanation: `Start with base cases Fib(0)=0, Fib(1)=1` });

  for (let i = 2; i <= n; i++) {
    steps.push({ type: 'table_access', indices: [i - 1, i - 2], explanation: `Calculating Fib(${i}) using Fib(${i - 1}) and Fib(${i - 2})` });
    dp[i] = dp[i - 1] + dp[i - 2];
    steps.push({ type: 'table_update', indices: [i], value: dp[i], explanation: `Fib(${i}) = ${dp[i]}` });
  }
  return steps;
};
