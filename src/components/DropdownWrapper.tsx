  ...
  return (
    <div className="flex flex-col gap-2 relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`
          flex items-center justify-between w-64 px-4 py-2.5 
          bg-slate-800 border border-slate-700 rounded-xl text-sm font-medium
          transition-all duration-200 outline-none
          ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-indigo-500 hover:bg-slate-750 cursor-pointer'}
      ...
      >
        ...
        <ChevronDown 
          size={18} 
          className={`text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180 text-indigo-400' : ''}`} 
        />
      </button>

      {isOpen && !disabled && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-slate-800 border border-slate-700 rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
